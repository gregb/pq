...
func openTestConn(t Fatalistic) *sql.DB {
	conn, err := openTestConnConninfo("user=pqgotest password=pqgotest")
	if err != nil {
		t.Fatal(err)
	}
	return conn
...
func TestParseComplete(t *testing.T) {
	tpc := func(commandTag string, command string, affectedRows int64, shouldFail bool) {
		...
		actualRowsAffected, c := parseComplete(commandTag)
		if c != command {
			t.Errorf("Expected %v, got %v", command, c)
		}

		if actualRowsAffected != affectedRows {
			t.Errorf("Expected %d, got %d", affectedRows, actualRowsAffected)
		}
	}
